        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16">
            <div className="flex items-center">
              ...
              >
                <div className="h-10 w-10 bg-gradient-to-tr from-green-600 to-green-400 rounded-lg flex items-center justify-center shadow-lg">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                  </svg>
                </div>
                <span className="ml-2 text-xl font-bold text-green-600">EXAIMA</span>
              </motion.div>
            </div>
                ...
                <AnimatePresence>
                  {showSettings && (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      exit={{ opacity: 0, y: -20 }}
                      className={`absolute right-0 mt-2 w-80 rounded-md shadow-lg ${darkMode ? 'bg-gray-800 border border-gray-700' : 'bg-white border border-gray-200'} divide-y ${darkMode ? 'divide-gray-700' : 'divide-gray-100'} origin-top-right z-30`}
                    >
                      ...
                      <div className="max-h-60 overflow-y-auto">
                        {notifications.length > 0 ? notifications.map(notification => (
                          <div 
                            key={notification.id} 
                            className={`px-4 py-3 ${notification.read ? '' : 'bg-green-50'} ${darkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-50'} cursor-pointer transition-colors duration-200`}
                ...
                <AnimatePresence>
                  {showProfileDropdown && (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      exit={{ opacity: 0, y: -20 }}
                      className={`absolute right-0 mt-2 w-56 rounded-md shadow-lg ${darkMode ? 'bg-gray-800 border border-gray-700' : 'bg-white border border-gray-200'} divide-y ${darkMode ? 'divide-gray-700' : 'divide-gray-100'} origin-top-right z-30`}
                    >
                      <div className="px-4 py-3">
        ...
        <AnimatePresence mode="wait">
            ...
            >
              {searchTerm && filteredExams.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="mt-1 text-sm text-gray-500">Try searching with different keywords or browse all exams.</p>
                  <div className="mt-6">
                    <button 
                ...
                <>
                  ...
                  {filteredExams.length === 0 && (
                    ...
                    >
                      ...
                      <p className="text-gray-400 mt-1">Please check back later or contact your administrator.</p>
                    </motion.div>
                  )}
                </>
              )}
          ...
          {activeTab === 'results' && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -20 }}
              transition={{ duration: 0.3 }}
              className={`bg-white p-6 rounded-xl shadow-lg ${darkMode ? 'bg-gray-800' : 'bg-white'}`}
            >
              ...
              <div className="text-center py-12">
                ...
                <p className={`mt-1 text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>Complete an exam to see your results here.</p>
                <div className="mt-6">
                  <button 
                    onClick={() => setActiveTab('exams')}
                    className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-green-600 hover:bg-green-700"
                ...
                <div className="mt-6 md:mt-0 md:ml-10 flex-1">
                  ...
                  <div className="mt-4 space-y-4">
                    <div>
                      ...
                      <div className={`mt-1 p-3 ${darkMode ? 'bg-gray-700' : 'bg-gray-50'} rounded-md`}>
          ...
          {activeTab === 'settings' && (
            ...
            >
              ...
              <div className="space-y-6">
                <div>
                  <h3 className={`text-lg font-medium ${darkMode ? 'text-gray-200' : 'text-gray-900'}`}>Appearance</h3>
                  <div className="mt-4 flex items-center">
                    <button
                      onClick={() => setDarkMode(false)}
                      className={`mr-4 p-4 rounded-lg ${!darkMode ? 'ring-2 ring-green-500' : 'border'} ${darkMode ? 'bg-gray-700 border-gray-600' : 'bg-white border-gray-200'}`}
                    >
                      <div className="bg-white rounded-md p-2 shadow-sm">
                        <div className="h-4 w-16 bg-gray-200 rounded mb-2"></div>
                        <div className="h-3 w-12 bg-gray-200 rounded"></div>
                      </div>
                      <p className={`mt-2 text-sm font-medium ${darkMode ? 'text-gray-300' : 'text-gray-800'}`}>Light Mode</p>
                    </button>
                    
                    <button
                      onClick={() => setDarkMode(true)}
                      className={`p-4 rounded-lg ${darkMode ? 'ring-2 ring-green-500' : 'border'} ${darkMode ? 'bg-gray-700 border-gray-600' : 'bg-white border-gray-200'}`}
                    >
                      ...
                      <p className={`mt-2 text-sm font-medium ${darkMode ? 'text-gray-300' : 'text-gray-800'}`}>Dark Mode</p>
                    </button>
                  </div>
                </div>
                
                <div>
                  <h3 className={`text-lg font-medium ${darkMode ? 'text-gray-200' : 'text-gray-900'}`}>Notifications</h3>
                  <div className="mt-4 space-y-3">
                    <div className="flex items-center">
                      <input
                        id="notifications-new-exams"
                        type="checkbox"
                        className="h-4 w-4 text-green-600 focus:ring-green-500 border-gray-300 rounded"
                        defaultChecked
                      />
                      <label htmlFor="notifications-new-exams" className={`ml-3 text-sm ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                        New available exams
                      </label>
                    </div>
                    <div className="flex items-center">
                      <input
                        ...
                        defaultChecked
                      />
                      <label htmlFor="notifications-results" className={`ml-3 text-sm ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                        Exam results
                      </label>
                    </div>
                    <div className="flex items-center">
                      <input
                        ...
                        defaultChecked
                      />
                      <label htmlFor="notifications-reminders" className={`ml-3 text-sm ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                        Study reminders
                      </label>
                    </div>
                  </div>
                ...
                <div>
                  <h3 className={`text-lg font-medium ${darkMode ? 'text-gray-200' : 'text-gray-900'}`}>Account</h3>
                  <div className="mt-4 space-y-4">
                    <button className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-green-600 hover:bg-green-700">
                      Change Password
                    </button>
                    <button className="ml-4 inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md shadow-sm text-red-700 bg-white hover:bg-gray-50">
                      Delete Account
                    </button>
                  </div>
                </div>
      ...
      <footer className={`${darkMode ? 'bg-gray-800 text-gray-400' : 'bg-gray-50 text-gray-500'} py-8 mt-12`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row justify-between items-center">
            <div className="flex items-center">
              <div className="h-8 w-8 bg-gradient-to-tr from-green-600 to-green-400 rounded-lg flex items-center justify-center shadow-md">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                </svg>
              </div>
              <span className="ml-2 text-sm font-medium">ExamMaster © 2025</span>
            </div>
